...
def fill_trainval_infos(data_path, nusc, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    for index, sample in enumerate(nusc.sample):
        ...
        # print("lidar_path: {}, lidarseg_labels_filename: {}".format(ref_lidar_path, lidarseg_labels_filename))
        # input()

        info = {
            'lidar_path': Path(ref_lidar_path).relative_to(data_path).__str__(),
            'lidar_sample_token': ref_sd_token,
        ...
        while len(sweeps) < max_sweeps - 1:
            if curr_sd_rec['prev'] == '':
                ...
                # if not test:
                #     lidarseg_labels_filename = nusc.get('lidarseg', curr_sd_rec['token'])['filename']
                # else:
                #     lidarseg_labels_filename = None
